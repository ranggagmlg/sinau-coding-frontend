...
function createRowACtion(item) {
    ...
    if (!item) {
        ...
        actions = '<button class="button-cancel btn btn-warning material-icons always-show" style="min-width: 24px" onclick="onEditOrCancel(this, \'CANCEL\')">navigate_before</button>' +
            '<button class="button-save btn btn-warning material-icons always-show" style="min-width: 24px" onclick="onSaveOrDelete(this, \'SAVE\')">navigate_next</button>';

        // Membuat row dengan default value empty
        rows = '<tr class="new-item">' +
            '<td>-</td>' +
            // Tampilkan sebagai input text dengan default shown
            '<td class="input-content always-show"><input class="rounded" style="background-color: #bec4cf" value="" autofocus></td>' +
            // Button actions dengan data-id empty
            '<td data-id="">' + actions + '</td>' +
            '</tr>';
    } else {
        // Membuat button actions conditional show
        actions = '<button class="button-edit btn btn-warning material-icons" style="min-width: 24px" onclick="onEditOrCancel(this, \'EDIT\')">settings_backup_restore</button>' +
            // Hanya tampil setelah klik edit
            '<button class="button-cancel btn btn-warning material-icons" style="min-width: 24px" onclick="onEditOrCancel(this, \'CANCEL\')">navigate_before</button>' +
            '<button class="button-save btn btn-warning material-icons" style="min-width: 24px" onclick="onSaveOrDelete(this, \'SAVE\')">navigate_next</button>' +
            // Selalu tampil
            '<button class="button-delete btn btn-primary material-icons" style="min-width: 24px" onclick="onSaveOrDelete(this, \'DELETE\')">delete</button>';

        // Membuat row dengan isi berdasarkan item
        rows = '<tr style="font-family: Rockwell">' +
            '<td>' + item.id + '</td>' +
